...
def export_html(content: dict[str, Any], template: str = "creator-launch") -> str:
    brand = content.get("brandName", "Landing Page")
    headline = content.get("headline", "")
    subheadline = content.get("subheadline", "")
    primary_cta = content.get("primaryCta", "Get Started")
    secondary_cta = content.get("secondaryCta", "Learn More")
    ...
    benefits_html = "".join(
        ...
        f'<h3 style="color:{colors["primary"]};margin-bottom:8px">{b.get("title","")}</h3>'
        f'<p style="color:{colors["text"]};opacity:0.8">{b.get("description","")}</p></div>'
        for b in benefits
    )

    features_html = "".join(
        f'<div style="background:{colors["card"]};padding:24px;border-radius:12px;flex:1;min-width:250px">'
        f'<h3 style="color:{colors["secondary"]};margin-bottom:8px">{f.get("title","")}</h3>'
        f'<p style="color:{colors["text"]};opacity:0.8">{f.get("description","")}</p></div>'
        for f in features
    )

    problem_points = "".join(f'<li style="margin-bottom:8px">{p}</li>' for p in problem.get("points", []))

    testimonials_html = "".join(
        f'<div style="background:{colors["card"]};padding:20px;border-radius:12px;margin-bottom:16px;'
        f'border-left:4px solid {colors["primary"]}"><p style="color:{colors["text"]};font-style:italic">{t}</p></div>'
        for t in social_proof.get("items", [])
    )

    pricing_features = "".join(
        f'<li style="margin-bottom:8px;color:{colors["text"]}">{pf}</li>' for pf in pricing.get("features", [])
    )

    faqs_html = "".join(
        f'<div style="margin-bottom:20px;padding:20px;background:{colors["card"]};border-radius:8px">'
        f'<h4 style="color:{colors["primary"]};margin-bottom:8px">{fq.get("question","")}</h4>'
        f'<p style="color:{colors["text"]};opacity:0.8">{fq.get("answer","")}</p></div>'
        for fq in faqs
    )
...
<title>{seo.get("title", brand)}</title>
...
<h2 style="font-size:36px;margin-bottom:16px">{problem.get("title","")}</h2>
...
<section>
<div class="container" style="text-align:center">
<h2 style="font-size:36px;margin-bottom:16px">{solution.get("title","")}</h2>
<p style="font-size:18px;opacity:0.8;max-width:800px;margin:0 auto">{solution.get("description","")}</p>
</div>
</section>
...
<section>
<div class="container">
<h2 style="font-size:36px;margin-bottom:32px;text-align:center">{social_proof.get("title","")}</h2>
...
<section>
<div class="container" style="text-align:center">
<h2 style="font-size:36px;margin-bottom:16px">{pricing.get("title","")}</h2>
<p style="font-size:18px;opacity:0.8;margin-bottom:24px">{pricing.get("description","")}</p>
...
<p style="font-size:32px;font-weight:bold;color:{colors["primary"]};margin-bottom:24px">{pricing.get("price","")}</p>
...
<div class="container">
<h2 style="font-size:36px;margin-bottom:16px">{final_cta.get("title","")}</h2>
<p style="font-size:18px;opacity:0.8;margin-bottom:32px">{final_cta.get("description","")}</p>
<a href="#" class="btn-primary">{final_cta.get("button","Get Started")}</a>
</div>
</section>
...
def export_react(content: dict[str, Any], template: str = "creator-launch") -> str:
    ...
    benefits_jsx = "\n".join(
        ...
        f'          <h3 className="text-purple-400 font-semibold mb-2">{b.get("title","")}</h3>\n'
        f'          <p className="text-slate-300">{b.get("description","")}</p>\n'
        f"        </div>"
    ...
    features_jsx = "\n".join(
        f'        <div key={{"{i}"}} className="bg-slate-800 p-6 rounded-xl">\n'
        f'          <h3 className="text-cyan-400 font-semibold mb-2">{f.get("title","")}</h3>\n'
        f'          <p className="text-slate-300">{f.get("description","")}</p>\n'
        f"        </div>"
        for i, f in enumerate(features)
    )

    problem_jsx = "\n".join(
        f'          <li key={{"{i}"}} className="text-slate-300 mb-2">{p}</li>'
    ...
    testimonials_jsx = "\n".join(
        ...
        f'          <p className="text-slate-300 italic">{t}</p>\n'
        f"        </div>"
    ...
    faqs_jsx = "\n".join(
        ...
        f'          <h4 className="text-purple-400 font-semibold mb-2">{fq.get("question","")}</h4>\n'
        f'          <p className="text-slate-300">{fq.get("answer","")}</p>\n'
        f"        </div>"
        for i, fq in enumerate(faqs)
    )

    pricing_features_jsx = "\n".join(
        f'            <li key={{"{i}"}} className="text-slate-300 mb-2">{pf}</li>'
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white">
      ...
      <section className="text-center py-24 px-6">
        ...
        <button className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-8 py-4 rounded-lg text-lg font-semibold mr-4">{primary_cta}</button>
        <button className="border-2 border-purple-500 text-purple-400 px-8 py-4 rounded-lg text-lg">{secondary_cta}</button>
      </section>

      {{/* Problem */}}
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-6">{problem.get("title","")}</h2>
...
{problem_jsx}
      ...
      <section className="py-20 px-6">
        <div className="max-w-4xl mx-auto text-center">
          <h2 className="text-3xl font-bold mb-6">{solution.get("title","")}</h2>
          <p className="text-lg text-slate-300">{solution.get("description","")}</p>
        </div>
      </section>
...
{features_jsx}
      ...
      {{/* Social Proof */}}
      <section className="py-20 px-6">
        <h2 className="text-3xl font-bold text-center mb-10">{social_proof.get("title","")}</h2>
...
{testimonials_jsx}
      ...
      <section className="py-20 px-6 text-center">
        <h2 className="text-3xl font-bold mb-4">{pricing.get("title","")}</h2>
        <p className="text-lg text-slate-300 mb-8">{pricing.get("description","")}</p>
        <div className="bg-slate-800 p-10 rounded-2xl max-w-md mx-auto border-2 border-purple-500">
          <p className="text-3xl font-bold text-purple-400 mb-6">{pricing.get("price","")}</p>
          <ul className="text-left mb-6">
{pricing_features_jsx}
          </ul>
          <button className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-8 py-4 rounded-lg text-lg font-semibold w-full">{primary_cta}</button>
        </div>
      </section>
...
{faqs_jsx}
      ...
      {{/* Final CTA */}}
      <section className="py-20 px-6 text-center bg-gradient-to-r from-purple-500/10 to-pink-500/10 rounded-3xl mx-6 mb-10">
        <h2 className="text-3xl font-bold mb-4">{final_cta.get("title","")}</h2>
        <p className="text-lg text-slate-300 mb-8">{final_cta.get("description","")}</p>
        <button className="bg-gradient-to-r from-purple-500 to-pink-500 text-white px-8 py-4 rounded-lg text-lg font-semibold">{final_cta.get("button","Get Started")}</button>
      </section>
